...
class Board(object):
	...
	def main(self):
		print('Welcome to this Python Chess Game. You can move by typing eg. "b2 b4"')
		while self.run == True:
			self.get_positions()
			self.print_board()
			self.destroyed_unit = ''
					...
					if self.turn == 'W': #whites turn
						...
						for i in self.white_players:
							if i.position == start_position:
								i.update_position(start_position, end_position, self.position_black_players, self.position_white_players, self.possible_moves_black, self.turn)
						...
						for i in self.black_players: #Update all possible moves of the enemy team
							i.update_possible_moves(self.position_white_players, self.position_black_players, self.possible_moves_white, self.turn)
						for i in self.white_players: #Update all possible moves of the own team
							i.update_possible_moves(self.position_black_players, self.position_white_players, self.possible_moves_black, self.turn)
						self.get_positions() 
						for i in self.possible_moves_black:
							if self.position_king_white in i: #If the own king is check  
								for i in self.white_players:
									if i.position == end_position:
										i.return_position(start_position, end_position, self.position_black_players, self.position_white_players, self.possible_moves_black, self.turn)
										for i in self.black_players:
											try:
											...
											for k in possible_positions_to_cover:
												for n in self.black_players:
													if k in n.possible_moves:
														...
														n.update_position(n.position, k, self.position_black_players, self.position_white_players, self.possible_moves_black, self.turn)
														...
														for x in self.white_players: 
															x.update_possible_moves(self.position_black_players, self.position_white_players, self.possible_moves_black, self.turn)
														for y in self.black_players: 
															y.update_possible_moves(self.position_white_players, self.position_black_players, self.possible_moves_white, self.turn)
														self.get_positions()
														for p in self.possible_moves_white:
															if self.position_king_black in p:
																who_can_help[(n.object_name, k)] = 1
														n.return_position(current_position, k, self.position_black_players, self.position_white_players, self.possible_moves_black, self.turn)
														...
														for z in self.white_players:
															z.update_possible_moves(self.position_black_players, self.position_white_players, self.possible_moves_black, self.turn)
														for w in self.black_players: 
															w.update_possible_moves(self.position_white_players, self.position_black_players, self.possible_moves_white, self.turn)
														self.get_positions()
						...
						for i in self.black_players:
							if i.position == start_position:
								i.update_position(start_position, end_position, self.position_white_players, self.position_black_players, self.possible_moves_white, self.turn)	
								for i in self.white_players:
									if i.position == end_position:
										self.destroyed_unit = i.object_name
										i.position = (-1,-1)								
						...
						for i in self.possible_moves_white:
							if self.position_king_black in i: 
								for i in self.black_players:
									if i.position == end_position:
										i.return_position(start_position, end_position, self.position_white_players, self.position_black_players, self.possible_moves_white, self.turn)
										for i in self.white_players:
											try:
												if i.object_name == self.destroyed_unit:
													i.position = end_position	
											...
											for k in possible_positions_to_cover: # Würde der Zug in dieses Feld den König aus dem Schach befreien?
												for n in self.white_players:
													if k in n.possible_moves:
														...
														current_object = n.object_name
														current_position = n.position
														n.update_position(n.position, k, self.position_white_players, self.position_black_players, self.possible_moves_white, self.turn)
														...
														for p in self.possible_moves_black:
															if self.position_king_white in p:
																who_can_help[(n.object_name, k)] = 1
														n.return_position(current_position, k, self.position_white_players, self.position_black_players, self.possible_moves_white, self.turn)
	...
	def check_chosen_move(self, ps, pe):
		if self.turn == 'W' and ps in self.position_white_players:
			try:
				for i in self.white_players:
					if i.position == ps:
						if i.check_if_move_legit(ps, pe, self.position_black_players, self.position_white_players, self.possible_moves_black, self.turn):
							return True
						else: 
			...
			try:
				for i in self.black_players:
					if i.position == ps:
						if i.check_if_move_legit(ps, pe, self.position_white_players, self.position_black_players, self.possible_moves_white, self.turn):
							return True
						else: 
							print('move not legit')
							return False
	...
	def get_positions(self):
		self.white_players = white_players = [self.B1,self.B2,self.B3,self.B4,self.B5,self.B6,self.B7,self.B8,self.T1,self.T2,self.S1,self.S2,self.L1,self.L2,self.D1,self.K1]
		...
		self.black_players = black_players = [self.B9,self.B10,self.B11,self.B12,self.B13,self.B14,self.B15,self.B16,self.T3,self.T4,self.S3,self.S4,self.L3,self.L4,self.D2,self.K2]
