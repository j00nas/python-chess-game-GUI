...
def save_game():
	...
	try:
		with open('savings.txt', 'w') as file:
			...
			for key, value in Chess_Board.Fig_Pos.items():
				file.write(str(key))
				file.write(';')
				file.write(str(value.name))
				file.write(';')
				file.write(str(value._color))
				file.write(';')
				file.write(str(value.position))
				file.write(';')
				file.write(str(value.poss_moves))
				file.write(';\n')
	except:
		print('Make a move first')

def load_game():
	global Chess_Board, current_color, history, start_count_rounds, check_mate, game_started, fields_dic
	...
	with open('savings.txt', 'r') as file:		
		for line in file:
			informations = line.split(';')[:-1]
			position = (int(informations[0][1]), int(informations[0][4]))
			figure = informations[1]
			color = int(informations[2])

			poss_moves = []

			for i in range(len(informations[4])-1):
				if informations[4][i] == '(':
					x = int(informations[4][i+1])
				if informations[4][i] == ')':
					y = int(informations[4][i-1])
					poss_moves.append((x,y))

			if figure == 'King':
				Chess_Board.Fig_Pos[position] = King(color, position, poss_moves)
				fields_dic[position]['text'] = '\u265A' if color == 0 else '\u2654'
			elif figure == 'Queen':
				Chess_Board.Fig_Pos[position] = Queen(color, position, poss_moves)
				fields_dic[position]['text'] = '\u265B' if color == 0 else '\u2655'
			elif figure == 'Rook':
				Chess_Board.Fig_Pos[position] = Rook(color, position, poss_moves)
				fields_dic[position]['text'] = '\u265C' if color == 0 else '\u2656'
			elif figure == 'Bishop':
				Chess_Board.Fig_Pos[position] = Bishop(color, position, poss_moves)	
				fields_dic[position]['text'] = '\u265D' if color == 0 else '\u2657'			
			elif figure == 'Pawn':
				Chess_Board.Fig_Pos[position] = Pawn(color, position, poss_moves)
				fields_dic[position]['text'] = '\u265F' if color == 0 else '\u2659'
			elif figure == 'Knight':
				Chess_Board.Fig_Pos[position] = Knight(color, position, poss_moves)
				fields_dic[position]['text'] = '\u265E' if color == 0 else '\u2658'
...
savegame_button = Button(tk, text='Save Game', bg='white', height=1, width=10, command=lambda: [save_game()])
savegame_button.config(font=(10), pady = 5, padx = 10)
savegame_button.grid(row=6, column=9)

loadgame_button = Button(tk, text='Load Game', bg='white', height=1, width=10, command=lambda: [load_game()])
loadgame_button.config(font=(10), pady = 5, padx = 10)
loadgame_button.grid(row=7, column=9)
